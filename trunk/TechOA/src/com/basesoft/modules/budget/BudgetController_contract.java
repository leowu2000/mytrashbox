...
public class BudgetController_contract extends CommonController {
	...
	protected ModelAndView doHandleRequestInternal(HttpServletRequest request,
		...
		String returnUrl = "b_contract.do?action=list&sel_year=" + sel_year + "&sel_name=" + URLEncoder.encode(sel_name, "UTF-8") + "&sel_pjcode=" + URLEncoder.encode(sel_pjcode, "UTF-8") + "&sel_empname=" + URLEncoder.encode(sel_empname, "UTF-8");
		
		if("frame".equals(action)){
			mv = new ModelAndView("modules/budget/frame_budget_contract");
			List listPj = budgetDAO.getProject();
			mv.addObject("listPj", listPj);
			return mv;
		}else if("list".equals(action)){
			mv = new ModelAndView("modules/budget/list_budget_contract");
